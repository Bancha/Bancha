 ...
 * @since         Bancha v1.0 // TODO vom Precompiler ausfüllen lassen
 * @license       MIT License (http://www.opensource.org/licenses/mit-license.php)
 * @author        Florian Eckerstorfer <f.eckerstorfer@gmail.com>
 * @author        Andreas Kern <andreas.kern@gmail.com>
 * @author        Roland Schuetz <mail@rolandschuetz.at>
 * @author        Kung Wong <kung.wong@gmail.com>
 * @version       0.0.1 // TODO vom Precompiler ausfüllen lassen
 *
 * For more information go to http://banchaproject.org 
 */
/*jslint browser: true, onevar: true, undef: true, nomen: true, eqeqeq: true, plusplus: false, bitwise: true, regexp: true, newcap: true, immed: true */
/*global Ext, window */
		...
		cb = function(result, event) { // TODO test this
			...
			if(data===null) {
				Ext.Error.raise({
					...
					result: result,
					event: event,
					msg: 'Bancha: The Bancha.preloadModelMetaData('+models+') did expect to to get the metadata from the server, instead got: '+result
				});
			}
			// ENDIF
	...
	createModel: function(modelName, modelConfig) {
		...
		if(!this.modelMetaDataIsLoaded(modelName)) {
			...
			Ext.Error.raise({
				...
				modelName: modelName,
				modelConfig: modelConfig,
				msg: 'Bancha: Couldn\'t create the model cause the metadata is not loaded yet, please you onModelReady instead.'
			});
			// ENDIF
			return false;
		}
		...
		if(Ext.ClassManager.isCreated(modelName)) {
			Ext.Error.raise({
				...
				modelName: modelName,
				modelConfig: modelConfig,
				msg: 'Bancha: Couldn\'t read metadata from server, idProperty is missing.'
			});
		}	
		// ENDIF
		...
		if(!Ext.isDefined(this.getModelMetaData(modelName).idProperty)) {
			if(window.console && Ext.isFunction(console.warn)) {
				window.console.warn(
					'Bancha: The model '+modelName+' is already defined, it is posible to overwrite this, '+
        ...
        safeDirectFn = function(stub,method,modelName) {
            ...
            if(Ext.isDefined(map[method]) && Ext.isDefined(stub[map[method]])) {
                return stub[map[method]];
            }
            return function() {
                Ext.Error.raise({
                    plugin: 'Bancha',
		...
		metaData = this.getModelMetaData(modelName);
				...
				if(config.update) {
					...
					if(config.withReset) {
						buttons.push({
	                    	...
	                    	text: 'Reset',
							scope: store,
	                    	handler: this.gridFunctiongridFunction.onReset
						});
					}
				}
